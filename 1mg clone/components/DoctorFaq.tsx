...
export function DoctorFaq({ compact = false }: { compact?: boolean }) {
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  return <section className={`${styles.faqSection} ${compact ? styles.compact : ''}`} aria-labelledby="faq-heading"><div className={styles.faqIntro}><h2 id="faq-heading">Frequently Asked Questions</h2><p>Have questions about online doctor consultations? Find answers to some of the most common questions below.</p></div><div className={styles.faqList}>{faqs.map(([question, answer], index) => { const isOpen = openFaq === index; const answerId = `doctor-faq-answer-${index}`; return <article className={`${styles.faqItem} ${isOpen ? styles.open : ''}`} key={question}><button type="button" className={styles.faqButton} onClick={() => setOpenFaq((current) => current === index ? null : index)} aria-expanded={isOpen} aria-controls={answerId}><span>{question}</span><ChevronDown size={20} aria-hidden="true" /></button><div id={answerId} className={styles.answerWrap} aria-hidden={!isOpen}><div className={styles.answer}><p>{answer}</p></div></div></article>; })}</div></section>;
}
