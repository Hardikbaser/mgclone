...
const data=await send('Runtime.evaluate',{expression:`JSON.stringify({title:document.title,text:document.body.innerText.slice(0,14000),images:[...document.images].map(i=>({src:i.currentSrc||i.src,alt:i.alt,w:i.naturalWidth,h:i.naturalHeight})),fonts:[...document.fonts].map(f=>f.family),resources:performance.getEntriesByType('resource').map(r=>r.name).filter(n=>/woff|ttf|png|jpg|jpeg|webp|svg/.test(n)),overflow:document.documentElement.scrollWidth>innerWidth})`,returnByValue:true});
